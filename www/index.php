...
class LoginForm extends TwoTunicsForm {
	...
	protected function Form_Run() {
		...
		if (QApplication::$User) QApplication::Redirect('/dashboard/');
	}
	...
	protected function Form_Create() {
		...
		$this->chkRemember->HtmlAfter = 'Remember Me';
	...
	protected function btnLogin_Click($strFormId, $strControlId, $strParameter) {
		$objUser = User::LoadByUsernamePassword(trim(strtolower($this->txtUsername->Text)), $this->txtPassword->Text);
		if (!$objUser) {
		...
		if ($this->chkRemember->Checked) {
	 		setcookie('username', $objUser->__get('Username'), time() + 60*60*24*14, '/', null);
	 		$this->lblDebug->Text = "set the cookie";
		} else {
			setcookie('username', null, 1, '/', null);
		}

		QApplication::Login($objUser);

		if (array_key_exists('r', $_GET) && $_GET['r'])
		QApplication::Redirect($_GET['r']);
		else
		QApplication::Redirect('/www/dashboard/');
