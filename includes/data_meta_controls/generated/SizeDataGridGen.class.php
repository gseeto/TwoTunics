<?php
	 ...
	 * listing a collection of Size objects.  It includes
	 ...
	 * 
	 */
	class SizeDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this Size datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the Size object, or it can be any QQNode extending from QQN::Size().
		 * 
		 ...
		 * sorting using two fields from the Size object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from Size
		...
		public function MetaDataBinder(QQCondition $objCondition = null, $objOptionalClauses = null) {
			...
			if ($this->Paginator) $this->TotalItemCount = Size::QueryCount($objCondition, $objClauses);
			...
			// Set the DataSource to be a Query result from Size, given the clauses above
			$this->DataSource = Size::QueryArray($objCondition, $objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a Size-based QQNode.
		 * It will also verify that it is a proper Size-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'size') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "size".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'Id': return QQN::Size()->Id;
				case 'Value': return QQN::Size()->Value;
				case 'Category': return QQN::Size()->Category;
				default: throw new QCallerException('Simple Property not found in SizeDataGrid content: ' . $mixContent);
