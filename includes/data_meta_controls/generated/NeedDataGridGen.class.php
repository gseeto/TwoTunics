<?php
	 ...
	 * listing a collection of Need objects.  It includes
	 ...
	 * 
	 */
	class NeedDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this Need datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the Need object, or it can be any QQNode extending from QQN::Need().
		 * 
		 ...
		 * sorting using two fields from the Need object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from Need
		...
		public function MetaDataBinder(QQCondition $objCondition = null, $objOptionalClauses = null) {
			...
			if ($this->Paginator) $this->TotalItemCount = Need::QueryCount($objCondition, $objClauses);
			...
			// Set the DataSource to be a Query result from Need, given the clauses above
			$this->DataSource = Need::QueryArray($objCondition, $objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a Need-based QQNode.
		 * It will also verify that it is a proper Need-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'need') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "need".');
				...
				case 'Charity': return QQN::Need()->Charity;
				case 'QuantityStillRequired': return QQN::Need()->QuantityStillRequired;
				default: throw new QCallerException('Simple Property not found in NeedDataGrid content: ' . $mixContent);
