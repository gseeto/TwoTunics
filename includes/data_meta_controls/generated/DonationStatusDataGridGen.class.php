<?php
	 ...
	 * listing a collection of DonationStatus objects.  It includes
	 ...
	 * 
	 */
	class DonationStatusDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this DonationStatus datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the DonationStatus object, or it can be any QQNode extending from QQN::DonationStatus().
		 * 
		 ...
		 * sorting using two fields from the DonationStatus object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from DonationStatus
		...
		public function MetaDataBinder(QQCondition $objCondition = null, $objOptionalClauses = null) {
			...
			if ($this->Paginator) $this->TotalItemCount = DonationStatus::QueryCount($objCondition, $objClauses);
			...
			// Set the DataSource to be a Query result from DonationStatus, given the clauses above
			$this->DataSource = DonationStatus::QueryArray($objCondition, $objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a DonationStatus-based QQNode.
		 * It will also verify that it is a proper DonationStatus-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'donation_status') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "donation_status".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'Id': return QQN::DonationStatus()->Id;
				case 'Value': return QQN::DonationStatus()->Value;
				default: throw new QCallerException('Simple Property not found in DonationStatusDataGrid content: ' . $mixContent);
