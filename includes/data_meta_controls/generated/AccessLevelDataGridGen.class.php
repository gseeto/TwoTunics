<?php
	 ...
	 * listing a collection of AccessLevel objects.  It includes
	 ...
	 * 
	 */
	class AccessLevelDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this AccessLevel datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the AccessLevel object, or it can be any QQNode extending from QQN::AccessLevel().
		 * 
		 ...
		 * sorting using two fields from the AccessLevel object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from AccessLevel
		...
		public function MetaDataBinder(QQCondition $objCondition = null, $objOptionalClauses = null) {
			...
			if ($this->Paginator) $this->TotalItemCount = AccessLevel::QueryCount($objCondition, $objClauses);
			...
			// Set the DataSource to be a Query result from AccessLevel, given the clauses above
			$this->DataSource = AccessLevel::QueryArray($objCondition, $objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a AccessLevel-based QQNode.
		 * It will also verify that it is a proper AccessLevel-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'access_level') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "access_level".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'Id': return QQN::AccessLevel()->Id;
				case 'Value': return QQN::AccessLevel()->Value;
				default: throw new QCallerException('Simple Property not found in AccessLevelDataGrid content: ' . $mixContent);
