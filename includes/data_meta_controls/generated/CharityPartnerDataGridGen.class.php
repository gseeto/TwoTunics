<?php
	 ...
	 * listing a collection of CharityPartner objects.  It includes
	 ...
	 * 
	 */
	class CharityPartnerDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this CharityPartner datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the CharityPartner object, or it can be any QQNode extending from QQN::CharityPartner().
		 * 
		 ...
		 * sorting using two fields from the CharityPartner object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from CharityPartner
		...
		public function MetaDataBinder(QQCondition $objCondition = null, $objOptionalClauses = null) {
			...
			if ($this->Paginator) $this->TotalItemCount = CharityPartner::QueryCount($objCondition, $objClauses);
			...
			// Set the DataSource to be a Query result from CharityPartner, given the clauses above
			$this->DataSource = CharityPartner::QueryArray($objCondition, $objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a CharityPartner-based QQNode.
		 * It will also verify that it is a proper CharityPartner-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'charity_partner') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "charity_partner".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'Id': return QQN::CharityPartner()->Id;
				case 'Name': return QQN::CharityPartner()->Name;
				case 'Description': return QQN::CharityPartner()->Description;
				case 'Street': return QQN::CharityPartner()->Street;
				case 'City': return QQN::CharityPartner()->City;
				case 'State': return QQN::CharityPartner()->State;
				case 'Zipcode': return QQN::CharityPartner()->Zipcode;
				case 'Phone': return QQN::CharityPartner()->Phone;
				case 'Email': return QQN::CharityPartner()->Email;
				case 'ContactPerson': return QQN::CharityPartner()->ContactPerson;
				default: throw new QCallerException('Simple Property not found in CharityPartnerDataGrid content: ' . $mixContent);
